...
const GamegooSocketContext = createContext<GamegooSocketContextType | null>(null);

interface GamegooSocketProviderProps {
  children: ReactNode;
}

export function GamegooSocketProvider({ children }: GamegooSocketProviderProps) {
  const SOCKET_ENDPOINT = process.env.PUBLIC_SOCKET_URL || '';
  const { user: authUser, isAuthenticated } = useAuth();
  const [isConnected, setIsConnected] = useState(false);
  const hasConnectedRef = useRef(false);
  const lastAuthErrorRef = useRef(false);
  const clearAuthErrorTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const handleDisconnect = (...args: unknown[]) => {
      ...
      const maybeAuthExpired = lastAuthErrorRef.current || reason === 'io server disconnect';
    ...
    const handleConnectError = (...args: unknown[]) => {
      ...
      const isAuthError =
        msg.includes('unauthorized') || msg.includes('401') || msg.includes('jwt');
      if (isAuthError) {
...
export const useGamegooSocket = () => {
  ...
  if (!context) {
    throw new Error('useGamegooSocket must be used within GamegooSocketProvider');
  }
  return context;
};
