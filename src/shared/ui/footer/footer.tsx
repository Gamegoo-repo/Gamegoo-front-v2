...
export default function Footer() {
  ...
  if (isMobile) {
    return (
      <footer className="flex w-full flex-col gap-8 p-5">
        <section className="flex w-full flex-col gap-4 mobile:w-[417px]">
  ...
  return (
    <footer className="flex w-full flex-col justify-start px-5 pb-8 mobile:flex-row mobile:px-0 tablet:pb-[100px] desktop:pb-[120px]">
      <section className="flex w-full flex-col gap-7 mobile:w-[417px]">
...
function FooterInfo() {
  return (
    <p className="regular-13 flex w-full flex-col gap-1.5 text-gray-500 mobile:gap-1">
      <a href="mailto:gamegoo0707@gmail.com" className="transition-colors hover:text-gray-700">
...
function FeedbackForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    emailjs
      .send(SERVICE_ID, TEMPLATE_ID, { feedback: enteredFeedback, date: dateKr }, PUBLIC_KEY)
      .then(
        (_result) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <p className="bold-14 text-gray-700">Gamegoo 팀에게 소중한 피드백을 전달해주세요!</p>
      <form className="flex items-center gap-3" onSubmit={handleSubmit}>
        <input
          name="feedback"
          placeholder="본 피드백은 서비스 개선에 큰 도움이 됩니다 :)"
          className={
            'regular-13 w-full rounded-lg border border-gray-500 px-3 py-2.5 shadow-none transition-all duration-100 ease-in-out outline-none placeholder:text-gray-500 focus:border-violet-300 mobile:w-[343px]'
          }
