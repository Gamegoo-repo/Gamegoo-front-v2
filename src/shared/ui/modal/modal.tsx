...
const Modal = forwardRef<HTMLDivElement, ModalProps>(
  (
    { className, isOpen, children, onClose, hasCloseButton = true, isBackdropClosable = true },
    forwardedRef
  ) => {
    ...
    return createPortal(
      ...
      <div onClick={handleBackdropClick} className="modal-backdrop z-[1000]">
        ...
        >
          {hasCloseButton && (
            <CloseButton
              className="absolute top-0 right-0 -translate-x-2 translate-y-2 hover:rounded-lg hover:bg-gray-300"
              onClose={onClose}
            />
          )}
          {children}
