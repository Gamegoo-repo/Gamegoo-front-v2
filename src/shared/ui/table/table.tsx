...
export default function Table<T extends { id: number }>({
  ...
  if (isLoading) {
    ...
    content = (
      <tr>
        <td colSpan={memoizedColumns.length} className="regular-16 py-10 text-center text-gray-700">
          게시된 글이 없습니다.
        </td>
      </tr>
    );
      ...
      {data?.length ? (
        <tbody>
          {data.map((row) => (
            ...
            >
              {memoizedColumns.map((column) => (
                <td key={column.header} className="h-[86px] px-3 pt-5 pb-3 first:pl-3 last:p-0">
                  {column.accessor(row)}
                </td>
              ))}
            </tr>
