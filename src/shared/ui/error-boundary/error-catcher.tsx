...
export const ErrorCatcher = ({ children }: React.PropsWithChildren) => {
  ...
  useEffect(() => {
    ...
    if (error.response?.data.code && error.response?.data.code in SERVER_ERROR_MESSAGES) {
      const message =
        SERVER_ERROR_MESSAGES[error.response?.data.code as keyof typeof SERVER_ERROR_MESSAGES];
      toast.error(message);
    }
