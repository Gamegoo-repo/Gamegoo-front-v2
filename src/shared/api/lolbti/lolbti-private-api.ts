...
export const saveLolBtiResult = async (
  ...
  const response = await privateApiClient.post<ApiResponse<SaveLolBtiResultResponse>>(
    BASE_PATH,
    request
  );
 ...
 */
export const getMyLolBtiResult = async (): Promise<GetMyLolBtiResultResponse> => {
  const response = await privateApiClient.get<ApiResponse<GetMyLolBtiResultResponse>>(BASE_PATH);
  return response.data.data;
};
...
export const getLolBtiRecommendations = async (
  ...
  const response = await privateApiClient.get<ApiResponse<GetMyLolBtiRecommendationsResponse>>(
