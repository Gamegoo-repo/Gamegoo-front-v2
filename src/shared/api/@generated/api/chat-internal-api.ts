 ...
 * @export
 */
export const ChatInternalApiAxiosParamCreator = function (
...
export const ChatInternalApiFp = function (configuration?: Configuration) {
	const localVarAxiosParamCreator =
		ChatInternalApiAxiosParamCreator(configuration);
	return {
		/**
		 * 새로운 채팅 메시지를 등록하는 API 입니다.
		 * @summary 채팅 메시지 등록 API
		...
		> {
			...
			const localVarOperationServerBasePath =
				operationServerMap["ChatInternalApi.addChat"]?.[
		...
		> {
			const localVarAxiosArgs = await localVarAxiosParamCreator.getChatroomUuid(
				memberId,
				options,
			);
			const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
			const localVarOperationServerBasePath =
				operationServerMap["ChatInternalApi.getChatroomUuid"]?.[
					localVarOperationServerIndex
				]?.url;
			return (axios, basePath) =>
				createRequestFunction(
