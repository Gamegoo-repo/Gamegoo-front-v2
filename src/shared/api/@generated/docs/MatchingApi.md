...
```typescript
import { MatchingApi, Configuration } from './api';
...
const { status, data } = await apiInstance.findMatching(matchingUuid, targetMatchingUuid);
```
...
```typescript
import { MatchingApi, Configuration, InitializingMatchingRequest } from './api';
...
const { status, data } = await apiInstance.successMatching(matchingUuid, targetMatchingUuid);
```
...
const { status, data } = await apiInstance.updateBothMatchingStatus(matchingUuid, status);
```
...
const { status, data } = await apiInstance.updateMatchingStatus(matchingUuid, status);
```
