...
```typescript
import { RiotApi, Configuration } from './api';
...
const { status, data } = await apiInstance.handleRSOCallback(code, state);
```
...
```typescript
import { RiotApi, Configuration, RiotJoinRequest } from './api';
...
const { status, data } = await apiInstance.joinByRSO(riotJoinRequest);
```
...
```typescript
import { RiotApi, Configuration, RiotVerifyExistUserRequest } from './api';
...
const { status, data } = await apiInstance.verifyRiot(riotVerifyExistUserRequest);
```
