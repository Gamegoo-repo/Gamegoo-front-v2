...
```typescript
import { ChatApi, Configuration } from './api';
...
const { status, data } = await apiInstance.enterChatroom(chatroomUuid);
```
...
const { status, data } = await apiInstance.exitChatroom(chatroomUuid);
```
...
const { status, data } = await apiInstance.getChatMessages(chatroomUuid, cursor);
```
...
const { status, data } = await apiInstance.readChatMessage(chatroomUuid, timestamp);
```
...
const { status, data } = await apiInstance.startChatroomByBoardId(boardId);
```
...
const { status, data } = await apiInstance.startChatroomByMemberId(memberId);
```
