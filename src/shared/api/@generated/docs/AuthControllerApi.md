...
const { status, data } = await apiInstance.getTestAccessToken1(memberId);
```
...
```typescript
import { AuthControllerApi, Configuration, RejoinRequest } from './api';
...
const { status, data } = await apiInstance.rejoinMember(rejoinRequest);
```
...
```typescript
import { AuthControllerApi, Configuration, RefreshTokenRequest } from './api';
...
const { status, data } = await apiInstance.updateToken(refreshTokenRequest);
```
