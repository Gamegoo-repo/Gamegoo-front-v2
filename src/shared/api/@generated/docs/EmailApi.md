...
```typescript
import { EmailApi, Configuration, EmailRequest } from './api';
...
const { status, data } = await apiInstance.sendEmail(emailRequest);
```
...
const { status, data } = await apiInstance.sendEmailWithCheckDuplication(emailRequest);
```
...
```typescript
import { EmailApi, Configuration, EmailCodeRequest } from './api';
...
const { status, data } = await apiInstance.verifyEmail(emailCodeRequest);
```
