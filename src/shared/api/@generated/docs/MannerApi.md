...
```typescript
import { MannerApi, Configuration, MannerInsertRequest } from './api';
...
const { status, data } = await apiInstance.addNegativeMannerRating(memberId, mannerInsertRequest);
```
...
const { status, data } = await apiInstance.addPositiveMannerRating(memberId, mannerInsertRequest);
```
...
```typescript
import { MannerApi, Configuration } from './api';
...
const { status, data } = await apiInstance.getMannerKeywordInfo(memberId);
```
...
const { status, data } = await apiInstance.getMannerLevelInfo(memberId);
```
...
const { status, data } = await apiInstance.getNegativeMannerRatingInfo(memberId);
```
...
const { status, data } = await apiInstance.getPositiveMannerRatingInfo(memberId);
```
...
```typescript
import { MannerApi, Configuration, MannerUpdateRequest } from './api';
...
const { status, data } = await apiInstance.updateMannerRating(mannerId, mannerUpdateRequest);
```
