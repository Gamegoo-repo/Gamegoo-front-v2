...
```typescript
import { BlockApi, Configuration } from './api';
...
const { status, data } = await apiInstance.blockMember(memberId);
```
...
const { status, data } = await apiInstance.deleteBlockMember(memberId);
```
...
const { status, data } = await apiInstance.getBlockList(page);
```
...
const { status, data } = await apiInstance.unblockMember(memberId);
```
