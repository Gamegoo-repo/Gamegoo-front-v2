...
```typescript
import { ChatInternalApi, Configuration, ChatCreateRequest } from './api';
...
const { status, data } = await apiInstance.addChat(memberId, chatroomUuid, chatCreateRequest);
```
...
```typescript
import { ChatInternalApi, Configuration } from './api';
...
const { status, data } = await apiInstance.getChatroomUuid(memberId);
```
