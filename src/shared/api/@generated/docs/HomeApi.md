...
```typescript
import { HomeApi, Configuration } from './api';
...
```typescript
import { HomeApi, Configuration, RiotUserInfo } from './api';
...
const { status, data } = await apiInstance.getMemberId(riotUserInfo);
```
...
const { status, data } = await apiInstance.getTestAccessToken(memberId);
```
...
const { status, data } = await apiInstance.joinTest(riotUserInfo);
```
...
const { status, data } = await apiInstance.refreshStats(memberId);
```
