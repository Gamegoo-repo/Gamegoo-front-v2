...
export class GamegooSocket {
  ...
  private async handleJwtExpired(...args: unknown[]): Promise<void> {
    ...
    try {
      ...
      const canRetry =
        this.socket?.connected && errorData?.data?.eventName && errorData?.data?.eventData;

      if (canRetry) {
        const originalEventData = errorData.data.eventData;
        if (typeof originalEventData !== 'object' || originalEventData === null) {
          console.error(`Cannot retry event "${eventName}" because its data is not an object.`);
          return; // 현재 재시도 중단
        }
