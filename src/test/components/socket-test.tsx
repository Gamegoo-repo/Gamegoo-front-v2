import { useSocketConnectionEvents, useSocketMessage, useSocketStatus } from '@/shared/api/socket';
...
function SocketTestInner() {
  ...
  return (
    <div className="bg-gray-50 rounded-lg border p-4">
      <h3 className="mb-2 text-lg font-bold">🔌 소켓 연결 테스트</h3>
      <div className="space-y-2 text-sm">
        <div>
          <span className="font-medium">인증 상태:</span>
          <span
            className={`ml-2 rounded px-2 py-1 text-xs ${
              isAuthenticated ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
            }`}
          >
        ...
        {isAuthenticated && (
          <>
            ...
            <div>
              ...
              <span className="ml-2">{isConnected ? '✅ 연결됨' : '❌ 연결 안됨'}</span>
            </div>
          </>
        )}
        {!isAuthenticated && (
          <div className="text-gray-600">🔒 로그인하면 소켓이 자동으로 연결됩니다</div>
        )}
      </div>
      <div className="mt-3 text-xs text-gray-600">💡 개발자 콘솔에서 소켓 이벤트를 확인하세요!</div>
    </div>
  );
}
