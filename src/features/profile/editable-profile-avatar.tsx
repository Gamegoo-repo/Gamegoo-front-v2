...
function EditableProfileAvatar({ variant }: EditableProfileAvatarProps) {
  ...
  const [localProfileId, setLocalProfileId] = useState<number>(userInfo?.profileImg ?? 0);
  ...
  const profileSizeClass = variant === 'sm' ? 'w-[60px] h-[60px]' : 'w-[180px] h-[180px]';

  const pencilSizeClass = variant === 'sm' ? 'w-[24px] h-[24px]' : 'w-[40px] h-[40px]';

  const pencilWrapperSizeClass = variant === 'sm' ? 'w-[28px] h-[28px]' : 'w-[48px] h-[48px]';
      ...
      <Popover>
        <PopoverTrigger asChild>
          <button type="button" className="relative" aria-label="프로필 변경">
            <UserProfile id={localProfileId} sizeClass={profileSizeClass} hasDropShadow />
            <div
              className={cn(
        ...
        <PopoverContent className="popover w-[calc(100vw-32px)] rounded-2xl border border-black/15 bg-black/70 p-4 text-white shadow-[0_4px_18px_rgba(0,0,0,0.25)] backdrop-blur-md mobile:w-[520px] mobile:p-6">
          <p className="bold-20 mb-4">프로필 이미지 선택</p>

          {/* ⬇️ grid 모바일 대응 */}
          <div className="grid grid-cols-3 gap-3 mobile:grid-cols-4 mobile:gap-4">
            {PROFILE_IDS.map((id) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      `flex h-[72px] w-[72px] items-center justify-center rounded-full border-2 mobile:h-[96px] mobile:w-[96px]`,
                      isSelected
                  ...
                  >
                    <img
                      ...
                      alt={`Profile ${id}`}
                      className="h-[56px] w-[56px] mobile:h-[76px] mobile:w-[76px]"
                    />
                  </div>

                  {isSelected && (
                    <span className="absolute top-[2px] left-[2px] inline-flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-violet-600 text-white shadow mobile:h-7 mobile:w-7">
                      ✓
                    </span>
                  )}
                </button>
