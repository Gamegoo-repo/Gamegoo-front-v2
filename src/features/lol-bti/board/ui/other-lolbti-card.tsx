    ...
    <div className="w-full cursor-pointer" onClick={() => onClick()}>
      ...
      <LolBtiCard
        ...
        header={
          <>
            <h3
              className={cn(
                'mb-4 text-sm leading-none font-medium text-gray-500',
                !compatibilityLevel && 'invisible'
              )}
            >
              {compatibilityLevel ? COMPATIBILITY_LABEL[compatibilityLevel] : '\u00A0'}
            </h3>
            <p className={'mb-6 flex flex-col items-center gap-1.5'}>
              <span className="text-[40px] leading-none font-[900] text-violet-600">
                {result.rollBtiType}
              </span>
              <span className="text-sm font-medium text-violet-600">{typeData.title}</span>
            </p>
          </>
        }
        footer={actions}
      >
        <div className="relative">
          {compatibilityLevel && (
            <div className="absolute top-0 right-0 translate-x-[25%] -translate-y-[25%]">
        ...
        <span className="mb-7 inline-block text-center text-xl font-bold text-black">
          {result.gameName}
        </span>
        <LolBtiChampionStats className="bg-white" champions={result.championStatsResponseList} />
      </LolBtiCard>
    </div>
  );
}
