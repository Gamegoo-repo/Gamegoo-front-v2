...
function UserProfileContent({
  ...
  return (
    <FlexBox direction="column" className="h-full w-full" gap={24}>
      ...
      />
      <div className="grid w-full grid-cols-1 grid-rows-[auto_auto_auto] gap-y-9 pb-20 mobile:grid-cols-[1fr_auto_auto] mobile:grid-rows-[minmax(264px,auto)_auto] mobile:gap-x-3">
        <MannerLevelCard
...
export default function UserProfileBottomSheet({
  ...
  const profileData = useUserProfileData(memberId);

  return (
    <BottomSheet
      title={
        profileData.data.profile ? `${profileData.data.profile.gameName}님의 프로필` : undefined
      }
