      ...
      {champions.length > 0 ? (
        champions.map((champion) => (
          <div key={champion.championId} className="flex flex-col items-center justify-center">
            ...
            <span
              className={cn(
                '-mt-1 flex min-w-10 items-center justify-center rounded-full px-1 text-center text-[11px] leading-normal font-bold text-white',
        ...
        <span className="text-xs text-gray-500">최근 플레이 정보가 없습니다.</span>
      )}
    </FlexBox>
  );
}
