...
export function useLolBtiRecommendations(params: PrivateRecommendationsParams): {
...
export function useLolBtiRecommendations(
  ...
  pages: GetLolBtiRecommendationsResponse[] | GetMyLolBtiRecommendationsResponse[] | undefined;
  isLoading: boolean;
  isError: boolean;
  fetchNextPage: () => void;
  isFetchingNextPage: boolean;
  ...
  const compatibilityOrder = type === 'private' ? params.compatibilityOrder : undefined;

  const { data, isLoading, isError, fetchNextPage, isFetchingNextPage, hasNextPage } =
    useInfiniteQuery({
      queryKey: [
        ...(type === 'private' ? lolbtiKeys.privateBoards() : lolbtiKeys.publicBoards()),
      ...
      getNextPageParam: (lastPage) => (lastPage.hasNext ? lastPage.nextCursorMemberId : undefined),
      staleTime: 5 * 60 * 1000,
    });
