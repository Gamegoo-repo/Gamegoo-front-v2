...
export default function LolBtiCompatibleCard({ type, title, quote }: LolBtiCompatibleCardProps) {
  return (
    <LolBtiMatchCard>
          ...
          {[...MOCK_USERS, ...MOCK_USERS].map((user, index) => (
            ...
            >
              ...
              <div className="flex flex-col flex-nowrap gap-0.5 font-medium text-white blur-[2px]">
                ...
                <span className="text-xs text-gray-500">{user.tag}</span>
              </div>
            </li>
          ))}
        </ul>
      ...
      <div className="w-full px-4">
        <button
          type="button"
          className="w-full cursor-pointer rounded-lg bg-violet-600 py-4 text-base leading-none font-bold text-white transition-all duration-300 hover:text-gray-300 active:scale-95"
        >
          {type} 유저와 게임하러가기
        </button>
      </div>
