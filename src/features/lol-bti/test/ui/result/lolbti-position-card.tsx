...
export function LolBtiPositionCard({ position, champions }: LolBtiPositionCardProps) {
  return (
    <div className="flex w-full flex-col items-center gap-2 rounded-[10px] border-1 border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] px-8 py-5">
      <span className="text-lg font-semibold text-[#bfbfbf]">{position}</span>
      <div className="flex w-full flex-col gap-2">
        {champions.map((champion) => (
          ...
          >
            ...
            />
            <span className="text-base font-medium text-[#bfbfbf]">{champion.name}</span>
          </div>
        ))}
      </div>
    </div>
