...
export default function LolBtiShareButton({ result }: { result: LolBtiResultType }) {
  ...
  return (
    <>
      <LolBtiShareModal isOpen={isOpen} onClose={() => setIsOpen(false)} result={result}>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-center text-xs text-white">URL 복사</span>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            <img alt="카카오톡 공유" src={KakaoShareImg} className="h-full w-full" />
          </button>
          <span className="text-center text-xs text-white">카카오톡</span>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-center text-xs text-white">X</span>
        </div>
      </LolBtiShareModal>
      <button
        type="button"
        onClick={handleShare}
        className="w-full cursor-pointer text-sm text-[#ccc] transition-all duration-300 hover:text-gray-300 hover:underline hover:underline-offset-4"
      >
        결과 공유하기
      </button>
    </>
