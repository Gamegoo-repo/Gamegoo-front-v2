...
import LolBtiCompatibleCard, { LolBtiIncompatibleCard } from './lolbti-compatible-card';
...
export default function LolBtiResultSection({
  ...
  return (
    <LolBtiLayout variant="result">
      <FlexBox direction="column" align="center" className="relative w-full p-7">
        <FlexBox direction="column" align="center" gap="md" className="w-full">
          ...
          <section className="flex w-full animate-appear flex-col gap-10 self-stretch rounded-[25px] border-2 border-[rgba(90,66,238,0.35)] bg-white/5 px-4 py-6 shadow-[0_0_40px_0_rgba(90,66,238,0.25)] backdrop-blur-[5px]">
            ...
            />
            <LolBtiStrengthWeakness strength={typeData.strengths} weakness={typeData.weaknesses} />
            <button
              onClick={async () => {
                if (resultId?.length) {
                  ...
                  toast.error('공유 링크 생성에 실패했습니다. 잠시 후 다시 시도해주세요.');
                }
              }}
              type="button"
              className="w-full cursor-pointer rounded-lg bg-linear-to-r from-violet-600 to-violet-500 py-4 text-base font-bold text-white transition-all duration-300 hover:text-gray-300 active:scale-95"
            >
              결과 공유하기
            </button>
            ...
            <section className="flex w-full flex-col items-center gap-12 rounded-2xl border-2 border-[#333] bg-[#1A1A1A] pt-8 pb-11">
              <div className="flex w-full flex-col items-center gap-4 px-[34px]">
                <h3 className="text-xl font-semibold text-white">👍 잘 맞는 유형</h3>
              ...
              <div className="flex w-full flex-col items-center gap-4 px-[34px]">
                <h3 className="text-xl font-semibold text-white">👎 안 맞는 유형</h3>
            ...
            <section className="flex w-full flex-col items-center gap-10 rounded-2xl border-2 border-[#333] bg-[#1A1A1A] px-8 py-10 pb-[72px]">
              <h3 className="text-center text-xl font-bold text-white [text-shadow:0_0_10px_rgba(90,66,238,0.40)]">
                내 성향 분석
              </h3>
              <LolBtiAxisResult axisKey="A/F" percentage={axisPercentages['A/F']} />
              <LolBtiAxisResult axisKey="D/S" percentage={axisPercentages['D/S']} />
              <LolBtiAxisResult axisKey="C/T" percentage={axisPercentages['C/T']} />
              <LolBtiAxisResult axisKey="I/B" percentage={axisPercentages['I/B']} />
            </section>
          </section>
        </FlexBox>
        <FlexBox
        ...
        >
          <h3 className="text-center text-xl font-bold text-white">추천 챔피언</h3>
        ...
        <FlexBox direction="column" align="center" className="mt-10 w-full px-6" gap={'lg'}>
          <span
            className={[
              'relative inline-block self-center text-center',
              'text-sm font-semibold text-[#bfbfbf]',
              'rounded-md bg-[#2a2a2a] px-2.5 py-1.5',
              'shadow-[0_8px_24px_rgba(0,0,0,0.2)]',
              'mt-3 animate-bubble-float',
              "before:absolute before:left-1/2 before:-translate-x-1/2 before:content-['']",
              'before:-bottom-2 before:h-2 before:w-3 before:bg-[#2a2a2a]',
              'before:[clip-path:polygon(0_0,100%_0,50%_100%)]',
            ].join(' ')}
          >
            <span className="text-[#00e6ff]">나와 잘맞는 친구</span>를 찾고싶다면?
          </span>
          <button
            type="button"
        ...
        <article className="pt-14 pb-20 text-center text-[15px] text-[#9A9A9A]">
      ...
      <div className="sticky right-0 bottom-0 left-0 flex w-full flex-col items-center gap-5 bg-[linear-gradient(180deg,rgba(23,22,28,0.00)_0%,#17161C_58.93%)] px-[50px] pt-[31px] pb-12 mobile:w-[440px]">
        {footer}
      </div>
    </LolBtiLayout>
  );
