...
export function LolBtiProfileCard({
  ...
  return (
    ...
    <article id="result-card" className="flex w-full flex-col items-center">
      ...
      <span className="mb-7 text-[22px] font-bold text-white">{title}</span>
      <span className="mb-9 text-lg font-medium text-[#4ECDC4]">"{quote}"</span>
      <img
        alt={`${type} ${title} 결과 이미지`}
        src={`/assets/images/results/${imageIndex}.png`}
        className="mb-7 size-[90%] rounded-xl object-cover"
      />
      <p className="w-full text-center text-lg break-keep text-[#CCC]">{description}</p>
    </article>
  );
}
