...
const collectAxisScores = (answers: ('A' | 'B')[]): AxisScores => {
  return answers.reduce<AxisScores>(
    (scores, answer, index) => {
      const [aAxis, bAxis] = LOL_BTI_QUESTIONS[index].axis as [AxisChar, AxisChar];
 ...
 */
export const calculateLolBtiResult = (answers: ('A' | 'B')[]): LolBtiResultType => {
 ...
 */
export const calculateAxisPercentage = (answers: ('A' | 'B')[], axisKey: LolBtiAxisKey): number => {
  const scores = collectAxisScores(answers);
