...
export default function TermsDetailModal() {
  ...
  return (
    <Modal
      ...
      isBackdropClosable
      className="h-full w-full rounded-lg bg-white p-5 mobile:h-[785px] mobile:w-[523px] mobile:rounded-[30px] mobile:px-9 mobile:py-8"
    >
      <section className="flex h-full w-full flex-col gap-2 mobile:gap-5">
        <div className="flex w-full items-center justify-between">
          <h2 className="flex items-center gap-0.5 text-base font-bold text-gray-800 mobile:text-xl">
            {label}
            <span className={cn('inline-block', required ? 'text-violet-600' : 'text-gray-500')}>
              ({required ? '필수' : '선택'})
            </span>
          </h2>

          <CloseButton className="p-0 hover:rounded-lg hover:bg-gray-300" onClose={closeModal} />
        </div>

        <article className="modal-scrollbar-custom w-full flex-1 overflow-y-scroll rounded-xl bg-gray-200 pt-[19px] pr-[14px] pl-[18px] text-sm text-gray-800">
          <Suspense
            fallback={
