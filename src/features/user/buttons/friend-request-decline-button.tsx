...
export default function FriendRequestDeclineButton({ userId }: { userId: number }) {
  const declineRequestMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData(userKeys.profile(userId), (old: OtherProfileResponse) => ({
        ...old,
  ...
  return (
    <Button
      variant={'black'}
      size="xl"
      className="bold-14 h-[45px] w-1/2 rounded-[6px] mobile:w-[150px] mobile:rounded-xl"
