...
function FriendList() {
  ...
  const { setChatroom, setChatDialogType, clearSystemData } = useChatDialogStore();
  ...
  const { mutate: toggleFavorite } = useMutation({
    mutationFn: (memberId: number) => api.private.friend.reverseFriendLiked(memberId),
    onSuccess: () => {
  ...
  const handleFriendClick = async (friend: FriendInfoResponse) => {
    ...
    try {
      // 친구와의 채팅방 시작
      const response = await api.private.chat.startChatroomByMemberId(friend.memberId);
      ...
      if (chatroomData?.uuid) {
        const chatroom: ChatroomResponse = {
          ...
          targetMemberName: chatroomData.gameName || friend.name || '알 수 없음',
          tag: chatroomData.tag || friend.tag || '',
          targetMemberImg: chatroomData.memberProfileImg || friend.profileImg || 0,
  ...
  const filteredFriends = friends.filter(
    (friend) => searchTerm === '' || friend.name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <>
      ...
      <div className="border-b border-gray-200 p-4">
        <div className="relative flex items-center rounded-lg bg-gray-100 pl-[32px]">
          <SearchIcon width="16" height="16" className="absolute top-[11px] left-[15px]" />
          <input
            type="text"
            placeholder="친구 검색하기"
            value={searchTerm}
