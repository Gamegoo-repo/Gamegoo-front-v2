...
export interface BoardFormData extends Omit<
  BoardInsertRequest,
  'mainP' | 'subP' | 'mike' | 'contents'
> {
  mainP?: Position;
  mike: Mike;
  subP?: Position;
...
export default function PostFormModal(props: PostFormModalProps) {
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      onClose={handleClose}
      className="w-full mobile:w-[555px]"
      ref={modalRef}
    >
      <form className="flex flex-col gap-5">
        <section className="flex flex-col gap-5 mobile:gap-[30px]">
          ...
          <fieldset className="w-full">
            <legend className="mb-1 text-[11px] font-medium text-gray-800 mobile:mb-1.5 mobile:text-sm mobile:font-semibold">
              포지션
            </legend>

            {/** 포지션 카드 두 개 묶는 div */}
            <div className="grid h-fit w-full grid-cols-[1fr_1fr] grid-rows-1 gap-x-2 mobile:h-[98px]">
              <Card
                padding="lg"
                rounded="lg"
                className="flex h-full flex-1 justify-center gap-3 bg-white mobile:gap-14"
              >
                <PositionSelector
                  label="주포지션"
                  onChangePosition={(newState) => handleChangeFormData('mainP', newState)}
                ...
                <PositionSelector
                  label="부포지션"
                  onChangePosition={(newState) => handleChangeFormData('subP', newState)}
              ...
              <Card
                padding="lg"
                rounded="lg"
                className="flex h-full flex-1 flex-col items-center justify-between bg-white mobile:gap-2"
              >
                <span className="text-xs text-[11px] font-medium text-gray-700 mobile:font-bold">
                  내가 찾는 포지션
                </span>
                <div className="flex h-full items-center gap-4">
                  <PositionSelector
                    onChangePosition={(newState) => {
                      if (newState) {
                        handleChangeFormData('wantP', [newState, ...formData.wantP.slice(1)]);
                      }
                    }}
                  ...
                  <PositionSelector
                    onChangePosition={(newState) => {
                      if (newState) {
                        handleChangeFormData('wantP', [...formData.wantP.slice(0, 1), newState]);
                      }
                    }}
            ...
            <div className="flex w-full flex-wrap items-center gap-2 gap-y-3">
              {formData.gameStyles.map((styleId, _idx) => {
                ...
                return style ? (
                  <span
                    key={styleId}
                    className="flex items-center justify-center gap-1 rounded-full bg-white px-1 py-0.5 text-[13px] text-gray-800 mobile:px-3 mobile:py-1 mobile:text-base mobile:font-medium"
                  >
                    {style.gameStyleName}
          ...
          <div className="flex flex-col gap-1 mobile:gap-2">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                handleChangeFormData<'mike'>('mike', checked ? 'AVAILABLE' : 'UNAVAILABLE')
              }
            />
          </div>
          ...
          <div className="group flex flex-col gap-2">
            ...
            <textarea
              ref={textareaRef}
              className={cn(
                'rounded-10 h-[70px] w-full resize-none border-1 border-gray-400 px-2.5 py-2 text-xs text-gray-700 transition-colors duration-150 focus:border-violet-400 focus:outline-none mobile:text-lg',
            ...
            />
            {contentError && <small className="text-red-500">{contentError}</small>}
            <span
              className={cn(
        ...
        <section className="modal-actions">
          <button
            disabled={
              !isFormValid || (props.mode === 'edit' && isEqual(props.postToEdit, formData))
            }
