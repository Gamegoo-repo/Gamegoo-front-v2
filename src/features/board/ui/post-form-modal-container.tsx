...
export default function PostFormModalContainer(props: PostFormModalContainerProps) {
  const { isOpen, onClose, mode } = props;

  const postId = mode === 'edit' ? props.postId : undefined;

  const { isPending: isPendingUserInfo, data: userInfo, isError: isUserError } = useFetchMyInfo();
  ...
  if (mode === 'create') {
    return <PostFormModal isOpen={isOpen} onClose={onClose} mode="create" userInfo={userInfo} />;
  }
