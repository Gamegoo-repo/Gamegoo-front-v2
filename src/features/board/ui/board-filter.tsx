  ...
  return (
    <section className="flex h-full gap-2">
      ...
      <PositionButtons
        ...
        onSelectPosition={(value) => updateFilter('position', value as Position)}
      />
    </section>
  );
}
...
function PositionButtons({ selectedPosition, onSelectPosition }: PositionButtonsProps) {
  return (
    <ul className="flex h-full w-[286px] overflow-hidden rounded-lg bg-gray-100">
      {POSITION_BUTTON_ITEMS.map((item) => {
        ...
        return (
          <li
            key={item.position}
            className="h-full flex-1 cursor-pointer border-r border-gray-300 transition-all ease-in-out last:border-none hover:bg-gray-300"
          >
            <button
              type="button"
              className={cn(
