...
export interface PositionSelectorProps extends Omit<
...
export default function PositionSelector({
  ...
  return (
    <div className={cn('flex w-[49px] flex-col items-center mobile:gap-2', className)}>
      {label && (
        <label
          htmlFor={label}
          className="text-xs text-[11px] font-medium text-gray-700 mobile:font-bold"
        >
          {label}
        </label>
      )}
      ...
      <Popover containerRef={containerRef}>
        <PopoverTrigger asChild>
          <div className="flex h-10 w-10 items-center justify-center mobile:h-12 mobile:w-12">
            {selectedItem ? (
              <button
                type="button"
                ...
                {...attributes}
              >
                <selectedItem.icon className="h-7 w-7 mobile:h-9 mobile:w-9" />
              </button>
            ) : (
              <button
                type="button"
                aria-label={label || title}
                className={cn(
                  'flex h-6 w-8 cursor-pointer items-center justify-center rounded-full bg-violet-100 transition-colors hover:bg-violet-200 mobile:h-8 mobile:w-12',
                  isError && 'ring-2 ring-red-500'
                )}
                {...attributes}
              >
      ...
      {supportingText && (
        <small className={cn('text-xs', isError ? 'text-red-500' : 'text-gray-500')}>
          {supportingText}
        </small>
      )}
    </div>
