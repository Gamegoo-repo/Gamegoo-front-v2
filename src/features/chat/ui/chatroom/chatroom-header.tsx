...
const ChatroomHeader = ({ enterData }: ChatroomHeaderProps) => {
  ...
  const MENU_ITEMS = useMemo(() => {
    ...
    if (!isFriend && !enterData?.data?.friendRequestMemberId) {
      items.push(<FriendAddMenuItem key="friend-add" userId={chatroom?.targetMemberId || 0} />);
    }
    ...
    if (isFriend) {
      items.push(
        <FriendDeleteMenuItem key="friend-delete" userId={chatroom?.targetMemberId || 0} />
      );
    }

    items.push(
      <ReportMenuItem key="report" userId={chatroom?.targetMemberId || 0} reportType="CHAT" />
    );
  ...
  return (
    <div className="flex h-[var(--chatroom-header-height)] px-[12px] pt-[12px]">
      <div className="flex w-full items-center gap-2 border-b border-violet-300 py-2 pl-2">
        <LeftArrowIcon
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex flex-col gap-[2px]">
              <div className="flex items-center gap-[2px]">
                <p className="font-medium text-gray-900">{chatroom?.targetMemberName}</p>
                <p className="regular-13 text-gray-600">#{chatroom?.tag}</p>
              </div>
              <button
                type="button"
                onClick={handleCopyRiotId}
                className="flex w-fit cursor-pointer items-center justify-center rounded-sm border border-gray-300 bg-gray-100 px-[2px] text-[11px] font-medium text-gray-600"
              >
                소환사명 복사
              </button>
              ...
              {isFriend && (
                <>
                  {isOnline && (
                    <div className="absolute -top-1 -right-2 h-[7px] w-[7px] rounded-full bg-green-500" />
                  )}
