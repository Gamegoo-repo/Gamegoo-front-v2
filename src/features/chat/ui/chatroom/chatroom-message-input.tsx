...
const ChatroomMessageInput = ({ enterData }: ChatroomMessageInputProps) => {
  ...
  const isBlocked = enterData?.data?.blocked || enterData?.data?.blockedByTarget;
  ...
  const submitMessage = () => {
    if (!chatroomUuid || !message.trim() || isBlocked || isBlind || !isConnected) {
      return;
    }
    ...
    <div className="relative h-[var(--chatroom-input-height)] w-full rounded-b-none bg-white md:rounded-b-[20px]">
      ...
      >
        ...
        <div className="mx-5 mb-5 flex items-center justify-between gap-5">
          <div className="text-xs text-violet-400">{message.length} / 1000</div>
          <button
            disabled={message.length === 0 || isBlocked || isBlind || isPending || !isConnected}
            type="submit"
            className={`rounded-[25px] px-5 py-2.5 text-base font-medium text-white transition-colors ${
              isBlocked || isBlind
