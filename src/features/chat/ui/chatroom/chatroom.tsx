...
const Chatroom = ({ enterData, isEntering, enterError }: ChatroomProps) => {
  ...
  const { isMannerModalOpen, mannerModalType, openMannerModal, closeMannerModal } =
    useChatDialogStore();
  ...
  useEffect(() => {
    ...
    const serverMessageCount = enterData.data?.chatMessageListResponse?.listSize ?? 0;
  ...
  const renderMessage = useCallback(
    (message: ChatMessage, index: number) => {
      ...
      const showProfileImage = shouldShowProfileImage(message, index, allMessages);
      ...
      if (showDate && message.timestamp) {
        ...
        elements.push(<ChatroomDateDivider key={`date-${key}`} date={dateString} />);
      }
  ...
  const renderMessages = useMemo(() => {
    return allMessages.flatMap((message, index) => renderMessage(message, index));
    ...
    if (currentMessageCount > previousMessageCount && previousMessageCount > 0) {
      ...
      requestAnimationFrame(() => {
        ...
        if (messageElements.length >= newMessageCount) {
          const firstNewMessageElement = messageElements[newMessageCount - 1] as HTMLElement;
  ...
  useLayoutEffect(() => {
    if (!isLoading && !isEntering && allMessages.length > 0 && !hasInitiallyScrolled) {
  ...
  if (error || enterError) {
    return (
      <div className="flex h-full items-center justify-center text-red-500">
        {enterError ? '채팅방 입장에 실패했습니다.' : '메시지를 불러오는데 실패했습니다.'}
      </div>
    );
  }

  return (
    <div className="flex h-[calc(100vh-var(--chatroom-header-height)-var(--chatroom-input-height))] flex-col tablet:h-[calc(var(--dialog-height)-var(--chatroom-header-height)-var(--chatroom-input-height))]">
      <div
        ...
        {allMessages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center px-4">
            <div className="text-center">
              <p className="regular-16 mb-2 text-gray-700">첫 메시지를 보내보세요!</p>
              <p className="regular-16 text-gray-700">대화를 시작해보세요.</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col">
            {hasNextPage && <div ref={sentinelRef} className="h-1" style={{ minHeight: '1px' }} />}
            {renderMessages}
