...
const parseSystemData = (data: unknown): Partial<ChatMessage> => {
  ...
  const systemType =
    typeof rawSystemType === 'string' ? Number.parseInt(rawSystemType, 10) : rawSystemType;
  const boardId =
...
export const useChatroomSocket = (chatroomUuid: string | null) => {
  ...
  useSocketMessage<ChatMessageEventData>('my-message-broadcast-success', (eventData) => {
    if (!isAuthenticated || !chatroomUuid) return;
  ...
  useSocketMessage<SystemMessageEventData>('chat-system-message', (eventData) => {
    if (!isAuthenticated || !chatroomUuid) return;
  ...
  useSocketMessage<SystemMessageEventData>('manner-system-message', (eventData) => {
    if (!isAuthenticated || !chatroomUuid) return;
