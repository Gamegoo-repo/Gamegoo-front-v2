...
export const useChatMessageFetcher = (chatroomUuid: string | null) => {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = undefined }) => {
      const response = await api.private.chat.getChatMessages(chatroomUuid || '', pageParam);
      return (
        response.data.data || {
          chatMessageList: [],
          hasNext: false,
...
export const useChatMessage = (chatroomUuid: string | null) => {
  ...
  const { data, isLoading, error, fetchNextPage, hasNextPage, isFetchingNextPage } = queryResult;
