...
export default function MannerEvaluationModal({
  ...
  const addMutation = useMutation({
    mutationFn: async (payload: MannerInsertRequest) => {
      ...
      if (isPositive) {
        return (await api.private.manner.addPositiveMannerRating(memberId, payload)).data;
      }
      return (await api.private.manner.addNegativeMannerRating(memberId, payload)).data;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (payload: MannerUpdateRequest) => {
      if (!existingData?.mannerRatingId) throw new Error('mannerRatingId is required');
      return (await api.private.manner.updateMannerRating(existingData.mannerRatingId, payload))
        .data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
  ...
  const canSubmit = selected.length > 0 && (isEditing || !existingData?.mannerRatingId);
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between">
        ...
        <button type="button" aria-label="닫기" onClick={onClose} className="p-1 hover:bg-gray-200">
          <span className="sr-only">닫기</span>
          <CloseButton
            className="absolute top-0 right-0 -translate-x-[24px] translate-y-[24px] hover:rounded-lg hover:bg-gray-300"
            onClose={onClose}
          />
        </button>
      </header>
