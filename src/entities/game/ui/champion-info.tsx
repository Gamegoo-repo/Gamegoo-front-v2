...
export default function ChampionInfo({
  ...
  const championInfo = (
    <div className="flex flex-col gap-3">
      <span className="text-sm font-bold mobile:text-lg mobile:font-semibold">{championName}</span>
  ...
  return (
    <Tooltip content={championInfo}>
      <div className="flex flex-col items-center justify-center">
        ...
        <span
          className={cn(
            '-mt-1 rounded-full px-1 text-center text-[11px] leading-none font-bold text-white',
...
function StatItem({ label, value, subText }: StatItemProps) {
  return (
    <>
      <dt className="text-[13px] font-bold whitespace-nowrap text-gray-500 mobile:text-sm">
        {label}
      </dt>
      <dd className="text-[13px] font-bold whitespace-nowrap text-gray-100 mobile:text-sm">
        {value}
      </dd>
      <dd className="bold text-[13px] whitespace-nowrap text-gray-100 mobile:text-sm">
        {subText || ''}
      </dd>
    </>
  );
