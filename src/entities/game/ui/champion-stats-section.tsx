...
const containerVariants = cva('flex flex-row w-full h-full', {
  variants: {
    variant: {
      ...
      modal: 'gap-0.5 mobile:gap-2 mobile:w-full mobile:items-center mobile:justify-start',
...
const labelVariants = cva('text-white font-bold', {
  variants: {
    variant: {
      ...
      profile: 'font-medium mobile:font-bold text-[11px] mobile:text-xs leading-normal',
    },
  },
  defaultVariants: {
    variant: 'board',
  ...
  return championList.length ? (
    <ul className={containerClassName}>
      {championList.map((champion) => (
        <li key={champion.championId} className="flex items-center justify-center">
          <ChampionInfo
            {...champion}
