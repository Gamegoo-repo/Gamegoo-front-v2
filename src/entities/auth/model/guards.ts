...
export function isValidOAuthStatus(value: unknown): value is OAuthStatus {
  return typeof value === 'string' && Object.values(OAuthStatus).includes(value as OAuthStatus);
}
...
export function isNeedSignup(response: AuthCallbackParams | null): response is NeedSignupParams {
  return response?.status === OAuthStatus.NEED_SIGNUP;
}

export function isError(response: AuthCallbackParams | null): response is ErrorParams {
