...
export const useChatStore = create<ChatState & ChatActions>((set, _get) => ({
  ...
  updateChatroom: (chatroom) =>
    set((state) => {
      const existingIndex = state.chatrooms.findIndex((room) => room.uuid === chatroom.uuid);
  ...
  incrementUnreadCount: (chatroomUuid) =>
    set((state) => {
      const existingRoomIndex = state.chatrooms.findIndex((room) => room.uuid === chatroomUuid);
  ...
  setFriendOffline: (friendId) =>
    set((state) => {
      const newOnlineFriends = state.onlineFriends.filter((id) => id !== friendId);
