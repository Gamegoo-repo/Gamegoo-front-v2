...
function RouteComponent() {
  ...
  const columns: Column<MyBoardListResponse & { id: number; idx: number }>[] = [
    {
      ...
      width: '4%',
      accessor: (row) => (
        <div className="text-center text-gray-800 mobile:text-base mobile:font-bold">{row.idx}</div>
      ),
    },
    {
      header: '소환사',
      ...
      accessor: (row) => (
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-block max-w-[13ch] overflow-hidden text-ellipsis whitespace-nowrap text-gray-800 mobile:text-base mobile:font-semibold">
              {row.gameName}
            </span>
            <span className="text-gray-500 mobile:text-sm mobile:font-semibold">#{row.tag}</span>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      width: '30%',
      accessor: (row) => (
        <div className="w-full rounded-lg border border-gray-400 bg-gray-100 p-2 text-gray-800 mobile:text-[13px]">
    ...
    },
    {
      ...
      width: '13%',
      accessor: (row) => (
        <div className="medium-16 inline-block w-full min-w-max text-center whitespace-nowrap text-gray-500">
  ...
  return (
    <div className="h-full w-full">
      <h2 className="bold-25 mb-4 border-b border-gray-200 pb-4">내가 작성한 글</h2>
      ...
      {!isLoading && !isError && (
        <div className="flex min-h-[500px] w-full flex-col gap-15 mobile:min-w-[700px]">
          <Table data={rows} columns={columns} ariaLabel="내가 작성한 글 목록" />
          {data && data.totalPage >= 1 && <MyPostsPagination totalPages={data.totalPage} />}
        </div>
      )}
