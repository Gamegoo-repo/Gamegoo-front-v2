...
function RouteComponent() {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-10 px-8 pb-32 mobile:px-0">
      {/* --- 1. 상단 필터 영역 --- */}
      <div className="flex gap-2">
        {hasLolBti && (
          <Dropdown
            selectedLabel={getSortTitle(search.sort) || '궁합순'}
            className="h-full w-[77px] mobile:w-[138px]"
        ...
        <Dropdown
          ...
          className="h-full w-[89px] mobile:w-[138px]"
      ...
      {/* --- 2. 카드 리스트 영역 --- */}
      <section className="flex w-full flex-1 flex-col items-center gap-x-5 gap-y-7 mobile:grid mobile:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {/* 2-1. 로딩 중일 때 스켈레톤 노출 */}
        {isLoading &&
        ...
        {!isLoading &&
          (hasLolBti ? <MyLolBtiResultCard result={myBtiResult} /> : <LolBtiTestCtaCard />)}
