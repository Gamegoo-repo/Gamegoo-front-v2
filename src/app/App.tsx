...
const CatImage = () => {
    ...
    if (isLoading) return <div className="text-center">🐱 고양이 사진 로딩 중...</div>;
    if (error) return <div className="text-red-500">에러: {error.message}</div>;

    return (
        <div className="text-center space-y-4">
            <img src={data?.url} alt="랜덤 고양이" className="mx-auto max-w-md rounded-lg shadow-lg" />
            <div>
    ...
    return (
        <QueryClientProvider client={queryClient}>
            ...
            <div className="border-t pt-6">
                <h2 className="text-2xl font-bold text-center mb-4">🐱 TanStack Query 테스트</h2>
                <CatImage />
            </div>
        </QueryClientProvider>
    );
