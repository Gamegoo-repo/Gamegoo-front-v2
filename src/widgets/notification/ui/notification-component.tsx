...
export default function NotificationComponent() {
  ...
  const [actionState, setActionState] = useState<'read' | 'delete' | null>(null);
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.private.notification.getNotificationListByPage(page);
      return data.data;
    },
  });
  ...
  const readMutation = useMutation({
    mutationFn: async (notificationId: number) => {
      const { data } = await api.private.notification.readNotification(notificationId);
      return data.data;
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const { data } = await api.private.notification.deleteMultipleNotifications({
        notificationIds: ids,
      });
      return data;
    },
  ...
  const renderModalDescription = () => {
    ...
    if (actionState === 'read')
      return isAll ? (
        <span className={`text-gray-900 ${fontClass}`}>모든 알림을 읽음 처리할까요?</span>
      ) : (
        <span className={`text-gray-900 ${fontClass}`}>선택한 알림을 읽음 처리할까요?</span>
      );
    ...
    if (actionState === 'delete')
      return (
        <FlexBox direction="column" align="center" justify="center">
          ...
          <span className={`text-gray-900 ${fontClass}`}>삭제된 알림은 복구할 수 없습니다.</span>
        </FlexBox>
      );
  ...
  return (
    <div className="h-full w-full">
      <h2 className="bold-25 mb-8 border-b border-gray-200 pb-4">알림</h2>
      ...
      <Modal
        ...
        className="h-[177px] w-[320px] p-0 mobile:p-0 md:h-[268px] md:w-[540px]"
      >
        <FlexBox direction="column" align="center" fullHeight>
          ...
          <footer className="w-full border-t border-gray-400">
            <FlexBox direction="row" align="center" justify="between" fullWidth>
              <Button
                variant="ghost"
                className="flex-1 rounded-none border-r border-gray-400 py-[26px]"
                onClick={() => {
                  setIsOpenModal(false);
                  setActionState(null);
                }}
              ...
              <Button variant="ghost" className="flex-1 py-[26px]" onClick={handleConfirmModal}>
                {actionState === 'read' ? (
