...
export const getColumns = (options?: {
  ...
  return [
      ...
      accessor: (row) => {
        return (
          <div className="flex h-fit items-center gap-2">
            <Link to={'/users/$userId'} params={{ userId: row.memberId.toString() }}>
    ...
    {
      width: '9%',
      accessor: (row) => <UserPositions mainPosition={row.mainP} subPosition={row.subP} />,
      header: '주/부 포지션',
    },
    {
      width: '10%',
    ...
    },
    {
      width: '17%',
      accessor: (row) => (
        <ChampionStatsSection championList={row.championStatsResponseList} variant="board" />
      ),
      header: '최근 선호 챔피언',
    },
    {
      ...
      accessor: (row) => (
        <div
          className={cn(
            'bold-16 inline-block w-full min-w-max text-center whitespace-nowrap',
