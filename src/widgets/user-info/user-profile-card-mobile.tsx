  ...
  return (
    <div className="flex w-full flex-col gap-5 rounded-lg bg-gray-100 p-5">
      <div className="flex w-full items-center gap-2">
        <UserProfile id={data.profileImg} sizeClass="w-[52px] h-[52px]" hasDropShadow />
        <p className="flex flex-col">
          <div className="flex items-center gap-1">
            <h3 className="text-base font-bold text-gray-800">{data.gameName}</h3>
            <MikeTag
              className="px-1.5 py-0 text-[9px] font-bold"
              isMikeAvailable={data.mike === 'AVAILABLE'}
            />
          </div>
          <span className="text-xs font-semibold text-gray-500">#{data.tag}</span>
        </p>
        <div className="flex flex-1 justify-end">{menu}</div>
      </div>
      <div className="grid w-full grid-cols-[1fr_1fr] gap-2">
        <RankInfo label="솔로랭크" tier={data.soloTier} rank={data.soloRank} variant={'profile'} />
        <RankInfo label="자유랭크" tier={data.freeTier} rank={data.freeRank} variant={'profile'} />
      </div>
      <div className="grid w-full grid-cols-[1fr_1fr] gap-2">
        ...
        <div className="flex w-full justify-center gap-4 rounded-[6px] bg-white px-5 pt-3 pb-2">
          <PositionCard
            ...
            title={'내가 찾는 포지션'}
            className="medium-11"
            positionIcons={wantPositions.length ? wantPositions : [getPositionIcon('ANY')]}
          />
        </div>
      </div>
      <div className="flex w-full flex-col gap-2">
        ...
        {data.gameStyleResponseList.length ? (
          <ul className="flex w-full flex-wrap gap-1">
            {data.gameStyleResponseList.map((style) => (
              <li
                ...
                className="semibold-13 inline-block rounded-full bg-white px-4 py-1 leading-none whitespace-nowrap text-gray-700"
              >
                {style.gameStyleName}
              </li>
            ))}
          ...
          <span className="medium-14 text-gray-400">선택한 게임 스타일이 없어요</span>
        )}
      </div>

      <div className="flex flex-col gap-1.5">
        <span className="semibold-14 text-gray-600">최근 선호 챔피언</span>
        <ChampionStatsSection championList={data.championStatsResponseList} variant="profile" />
      </div>
