...
export default function UserProfileModal({ memberId, isOpen, onClose }: UserProfileModalProps) {
  ...
  const renderContent = () => {
    ...
    if (isError || !data?.profile || !data?.mannerLevel || !data?.mannerKeywords || !recentStats) {
      return (
        <div className="flex h-full w-full items-center justify-center pt-[68px]">
          <div className="text-center">
            <h2 className="mb-2 text-2xl font-bold text-gray-800">사용자를 찾을 수 없습니다</h2>
    ...
    return (
      <UserProfileCard
        data={data.profile}
        button={
          <UserActionButtons userId={data.profile.id} relationshipStatus={relationshipStatus} />
        }
        menu={<UserActionMenu userId={data.profile.id} relationshipStatus={relationshipStatus} />}
      />
    );
  };
  ...
  return (
    /** TODO: 디자인 시스템에 추가하기 */
    <Modal
      className="h-[90%]! overflow-hidden! p-0! xl:max-w-[1300px]"
      isOpen={isOpen}
      onClose={onClose}
    >
      <section className="flex h-full w-full flex-col gap-9 overflow-y-auto p-8">
        {data.profile && relationshipStatus && (
          <UserProfileHeader relationshipStatus={relationshipStatus} showBackButton={false}>
            {relationshipStatus === 'me' ? '나의 프로필' : `${data.profile.gameName}님의 프로필`}
          </UserProfileHeader>
        )}
        {renderContent()}
        {data.profile && data.mannerLevel && data.mannerKeywords && recentStats && (
          <div className="grid w-full grid-cols-1 grid-rows-[auto_auto_auto] gap-y-9 pb-20 mobile:grid-cols-2 mobile:gap-x-5 tablet:grid-cols-[1fr_auto_auto] tablet:grid-rows-[minmax(264px,auto)_auto] tablet:gap-x-3">
            ...
            <div className="w-full mobile:col-span-2 tablet:col-span-1">
              <RecentStatsCard
