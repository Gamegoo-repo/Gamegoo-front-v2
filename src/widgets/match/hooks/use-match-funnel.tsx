...
export interface UseMatchFunnelReturn {
  ...
  toStep: (
    step: FunnelStep,
    newState?: Partial<
      Pick<UseMatchFunnelReturn, 'type' | 'gameMode' | 'profile' | 'matchComplete'>
    >
  ) => void;
}
...
interface MatchFunnelStore {
  ...
  setStep: (step: FunnelStep, newState?: Partial<Omit<MatchFunnelStore, 'setStep'>>) => void;
}
...
export const useMatchFunnel = (): UseMatchFunnelReturn => {
  ...
  const { step, type, gameMode, profile, matchComplete, setStep } = useMatchFunnelStore();
