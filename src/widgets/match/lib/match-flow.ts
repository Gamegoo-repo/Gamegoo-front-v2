...
class MatchFlow {
  ...
  private phase: 'idle' | 'searching' | 'found' | 'completing' | 'completed' | 'cancelled' = 'idle';
  ...
  private ensureSocketBinding(): void {
    ...
    matchEventManager.on('matching-found-sender', forward('matching-found-sender'));
    matchEventManager.on('matching-found-receiver', forward('matching-found-receiver'));
    ...
    matchEventManager.on('matching-success-sender', forward('matching-success-sender'));
  }

  private handleInternalStateByEvent(event: MatchEventName) {
    // 서버 이벤트가 와도 UI/phase가 꼬이지 않게 최소 동기화
    if (event === 'matching-found-sender' || event === 'matching-found-receiver') {
  ...
  private emit<E extends MatchEventName>(event: E, payload: MatchEventPayloadMap[E]): void {
  ...
  on<E extends MatchEventName>(
    ...
    if (set) {
      ...
      this.listeners.set(event, new Set([listener as unknown as (payload: unknown) => void]));
    }
  ...
  private handleConnect(): void {
    ...
    if (!this.didSendInitialRequest) {
      ...
      const threshold = this.currentThreshold ?? this.lastRequest.threshold ?? 0;
  ...
  private flushPendingOutgoing(): void {
    ...
    const toFlush = this.pendingOutgoing.filter((p) => p.sessionId === currentSessionId);
    // flush 후 현재 세션 것만 제거
    this.pendingOutgoing = this.pendingOutgoing.filter((p) => p.sessionId !== currentSessionId);
  ...
  start(request: MatchingRequest & { memberId?: number }): boolean {
    ...
    if (this.phase === 'searching' || this.phase === 'found' || this.phase === 'completing') {
      return false;
    }
    ...
    if (this.phase === 'searching' || this.phase === 'found') {
      ...
      this.pendingCancelTimer = setTimeout(() => {
        ...
        const sameSession =
          this.pendingCancelSessionId !== null &&
          this.pendingCancelSessionId === this.currentSessionId;
        if (sameSession && (this.phase === 'searching' || this.phase === 'found')) {
