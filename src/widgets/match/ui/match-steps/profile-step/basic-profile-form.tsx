...
export default function BasicProfileForm({ funnel, containerRef }: BasicProfileFormProps) {
  ...
  const gameModeItems = useMemo(() => {
    ...
    return allModes.filter(
      (item) => item.id === 'SOLO' || item.id === 'FREE' || item.id === 'FAST' || item.id === 'ARAM'
    );
  }, []);
  ...
  const handleGameStyleToggle = (styleId: number) => {
    ...
    const newGameStyleResponseList = newIds
      ...
      .filter((item): item is { gameStyleId: number; gameStyleName: string } => item !== null);
    ...
    <>
      <div className="flex flex-col gap-2">
        ...
        <Dropdown
          ...
          variant="secondary"
          selectedLabel={currentGameMode ? getGameModeTitle(currentGameMode) : '선택'}
          onSelect={(value) => handleGameModeChange(value as GameMode | undefined)}
          items={gameModeItems}
        />
      </div>
      {currentGameMode !== 'ARAM' && (
        <div>
          ...
          <div className="flex min-h-[104px] w-full flex-col gap-[12px] tablet:flex-row">
            <div className="h-full flex-1 rounded-[10px] bg-white px-11 py-4">
              <ul className="flex h-full w-full justify-between gap-[8px]">
                <li className="flex h-full w-[49px] flex-col items-center justify-between">
                  ...
                  <PositionSelector
                    onChangePosition={(newState) => handlePositionChange('mainP', newState)}
                ...
                <li className="flex h-full w-[49px] flex-col items-center justify-between">
                  ...
                  <PositionSelector
                    onChangePosition={(newState) => handlePositionChange('subP', newState)}
            ...
            <div className="flex h-full flex-1 flex-col items-center justify-between rounded-[10px] bg-white px-11 py-4">
              ...
              <ul className="flex w-full items-end justify-between gap-4 tablet:justify-center">
                <li className="flex flex-col items-center justify-between">
                  <PositionSelector
                    onChangePosition={(newState) => handlePositionChange('wantP', newState, 0)}
                ...
                <li>
                  <PositionSelector
                    onChangePosition={(newState) => handlePositionChange('wantP', newState, 1)}
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <Switch checked={currentMike === 'AVAILABLE'} onCheckedChange={handleMikeChange} />
      </div>
    </>
  );
}
