...
function ProfileStep({ funnel, user }: ProfileStepProps) {
  ...
  const handleMatchStart = () => {
    ...
    const normalizedProfile =
      matchType === 'PRECISE'
        ? {
            ...
            gameStyleResponseList: currentProfile.gameStyleResponseList ?? undefined,
          }
        : {
            ...currentProfile,
            mike: currentMike,
    ...
    const matchingData = {
      ...
      wantP: funnel.type === 'PRECISE' ? mapPreciseWantPositions(profile.wantP) : [],
      gameStyleIdList: (() => {
        const ids = profile.gameStyleResponseList?.map((s) => s.gameStyleId) || [];
  ...
  return (
    <>
      ...
      />
      <div
        className="flex w-full items-center justify-center pt-[110px] mobile:pt-0"
        ref={containerRef}
      >
        <div className="w-full max-w-[1440px] px-[80px] pt-[60px] mobile:px-[20px] mobile:pt-[24px]">
          <div className="mt-[72px] mb-[150px] flex w-full flex-col items-center gap-4 mobile:mt-[15px]">
            <div
              className={cn(
