  ...
  useEffect(() => {
    ...
    const handleMatchingSuccess = async (_res: unknown) => {
      ...
      try {
        ...
        const chatroomUuid = payload?.data?.chatroomUuid ?? payload?.chatroomUuid ?? null;

        if (!chatroomUuid) return;

        const opponent = payload?.data?.opponent ?? payload?.opponent ?? matchComplete?.opponent;
        ...
        const { openDialog, setChatDialogType, setChatroom } = useChatDialogStore.getState();
    ...
    <>
      ...
      <div className="flex w-full items-center justify-center">
        <div className="w-full max-w-[1440px]">
          <div className="mt-[30px] mb-[150px] flex w-full flex-col items-center justify-center px-[10px] mobile:mt-[70px] mobile:px-[0px]">
            <div className="flex w-full flex-col items-center gap-[40px] mobile:grid mobile:grid-cols-2 mobile:items-start mobile:justify-items-center mobile:gap-[24px]">
              <MatchStartProfile user={{ ...authUser, ...funnel.profile }} />

              <MatchStartProfile
                user={matchComplete?.opponent as Partial<OpponentProfilePayload>}
                opponent
              />

              <div className="hidden mobile:block" />

              <div className="flex w-full max-w-[560px] flex-col items-center gap-4 mobile:items-stretch">
                <div className="text-center text-base font-semibold text-gray-700 mobile:text-lg">
                  {isMatched
                    ? '매칭 완료'
                    : timeLeft > 0
