...
const messagesWithTierN = Object.freeze(['나와 같은 티어의 n명이 매칭 중이에요!']);
...
function MatchLoadingCard({
  ...
  const showMessage = () => {
    ...
    setTimeout(() => {
      const totalMessages = [...messagesWithTierN, ...messagesWithTotalN, ...messagesWithoutN];
      ...
      if (messagesWithTierN.includes(randomMessage)) {
        setCurrentMessage(randomMessage.replace(/n/g, tierUserCount.toString()));
      } else if (messagesWithTotalN.includes(randomMessage)) {
        setCurrentMessage(randomMessage.replace(/n/g, totalUserCount.toString()));
      } else {
  ...
  return (
    <div className="animate-fade-in flex h-auto w-full max-w-[560px] flex-col items-center justify-center gap-6 rounded-[24px] bg-gray-100 px-6 py-8 text-gray-800 transition-opacity duration-500 md:h-[560px] md:gap-[42px] md:rounded-[30px] md:p-[30px_80px]">
      {/* 하트 애니메이션 */}
      ...
      <div className="flex flex-col items-center gap-2 md:gap-[4px]">
        ...
        <div
          className={clsx(
            `md:regular-20 flex h-[48px] items-center justify-center text-center text-sm transition-opacity duration-300 mobile:text-2xl md:h-[60px]`,
            textVisible ? 'animate-fade-in opacity-100' : 'animate-fade-out opacity-0'
          )}
        >
          {currentMessage}
        </div>
        {/* 시간 표시 */}
        <div className="md:light-32 text-lg text-gray-700 mobile:text-2xl">
          <span className="md:bold-32 font-bold text-violet-600">{formatTime(timeLeft)}&nbsp;</span>
          / 5:00
        </div>
      </div>
    </div>
