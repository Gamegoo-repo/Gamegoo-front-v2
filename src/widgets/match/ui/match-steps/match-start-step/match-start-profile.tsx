...
import type { MyProfileResponse, OtherProfileResponse, Position } from '@/shared/api';
...
import type { GameStyleItem, OpponentProfilePayload } from '@/widgets/match/lib/matching-types';
...
function MatchStartProfile({ user, opponent = false }: MatchStartProfileProps) {
  ...
  const { data: userMannerInfo } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.private.manner.getMannerKeywordInfo(memberId as number);
  ...
  const WantPositionIcon = getPositionIcon((user.wantP?.[0] as Position) ?? 'ANY');
  const WantPositionIcon2 = getPositionIcon((user.wantP?.[1] as Position) ?? 'ANY');
  const isMicOn = user.mike === 'AVAILABLE';

  const mannerLevel =
    opponent && typeof (user as Partial<{ mannerLevel: number }>).mannerLevel === 'number'
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 text-center md:gap-2">
        <h2 className="text-xl font-bold text-gray-900 md:text-3xl">{user.gameName}</h2>
        <p className="text-sm text-gray-500 md:text-base">#{user.tag}</p>
      </div>

      {/* 랭크 정보 */}
      <div className="mb-4 flex items-center justify-center gap-4 md:mb-8 md:gap-6">
        <div className="flex items-center gap-1 md:gap-2">
          <span className="text-xs font-medium text-gray-700 md:text-sm">솔로랭크</span>
          <span className="text-xs text-gray-500 md:text-sm">{user.soloTier}</span>
        </div>

        <div className="h-4 w-px bg-gray-300 md:h-5" />

        <div className="flex items-center gap-1 md:gap-2">
          <span className="text-xs font-medium text-gray-700 md:text-sm">자유랭크</span>
          <span className="text-xs text-gray-500 md:text-sm">{user.freeTier}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-1 md:gap-[6px]">
        <div className="scale-90 md:scale-100">{isMicOn ? <MicOnIcon /> : <MicOffIcon />}</div>
        <span
          className={clsx('md:semi-bold-13 text-xs', isMicOn ? 'text-violet-600' : 'text-gray-500')}
        >
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {user.gameStyleResponseList?.map((style: GameStyleItem) => (
          <span
            ...
            className="md:semi-bold-13 rounded-full bg-violet-200 px-3 py-1 text-xs text-gray-700 md:px-4 md:py-2"
          >
            {style.gameStyleName}
          </span>
        ))}
      ...
      <div className="flex w-full gap-2 md:gap-[12px]">
        <div className="flex-1 rounded-[10px] bg-gray-100 px-4 py-3 md:px-11 md:py-4">
          <ul className="flex h-full w-full justify-between gap-2">
            <li className="flex flex-col items-center justify-between">
              <span className="md:bold-12 text-[11px] text-gray-700">주 포지션</span>
              <MainPositionIcon className="w-9 text-gray-700 md:w-12" />
            </li>

            <li className="flex flex-col items-center justify-between">
              <span className="md:bold-12 text-[11px] text-gray-700">부 포지션</span>
        ...
        <div className="flex flex-1 flex-col items-center justify-between rounded-[10px] bg-gray-100 px-4 py-3 md:px-11 md:py-4">
          <span className="md:bold-12 text-[11px] text-gray-700">내가 찾는 포지션</span>
